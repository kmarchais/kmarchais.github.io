...
import { Vector3, ShaderMaterial, PerspectiveCamera as ThreePerspectiveCamera } from "three";
...
const TPMSShader = ({ params }: TPMSShaderProps) => {
  ...
  useFrame(({ clock }) => {
    ...
    material.current.uniforms.uOrthoScale.value =
      Math.max(camDist * Math.tan((cam.fov * Math.PI) / 360), 0.001);
    ...
    const cmapIndex: Record<string, number> = {
      "Blue-White-Red": 0.0,
      "Viridis": 1.0,
      "Plasma": 2.0,
      "Magma": 3.0,
    ...
    material.current.uniforms.uColormap.value = cmapIndex[params.colormap] ?? 0.0;
    ...
    material.current.uniforms.uSurfaceType.value = surfaceIndex[params.surface] ?? 0.0;
  });
